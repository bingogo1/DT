 ...
 * for <code>Hashtable</code> in <code>G11nLogFactory</code>. This application requires
 ...
 * for the <code>Hashtable</code> used in <code>G11nLogFactory</code> for J2EE enviroments
 * running 1.3+ JVMs. Use of this class <i>in most cases</i> (see below) will
 * allow classloaders to be collected by the garbage collector without the need 
 * to call {@link bin.g11n.cil.common.logger.G11nLogFactory#release(ClassLoader) G11nLogFactory.release(ClassLoader)}.
 * </p>
 *
 * <p><code>bin.g11n.cil.common.logger.G11nLogFactory</code> checks whether this class
 * can be supported by the current JVM, and if so then uses it to store
 * references to the <code>G11nLogFactory</code> implementationd it loads 
 ...
 * for method <code>G11nLogFactory.createFactoryStore</code> for more details.</p>
 *
 ...
 * the component loads a G11nLogFactory instance via the component classloader
 * a reference to it gets stored in the static G11nLogFactory.factories member,
 ...
 * <code>G11nLogFactory</code>'s factories member! If <code>G11nLogFactory.release()</code>
 ...
 * bundles g11n_logging by default. However, holding the classloader
 ...
 * </p>
 * 
 * <p> If the abstract class <code>G11nLogFactory</code> is 
 * loaded by the container classloader but a subclass of 
 * <code>G11nLogFactory</code> [LogFactory1] is loaded by the component's 
 * classloader and an instance stored in the static map associated with the
 * base G11nLogFactory class, then there is a strong reference from the G11nLogFactory
 * class to the G11nLogFactory1 instance (as normal) and a strong reference from
 * the G11nLogFactory1 instance to the component classloader via
 * <code>getClass().getClassLoader()</code>. This chain of references will prevent 
 * collection of the child classloader.</p>
 *
 * <p>
 * Such a situation occurs when the g11n_logging.jar is
 * loaded by a parent classloader (e.g. a server level classloader in a
 * servlet container) and a custom <code>G11nLogFactory</code> implementation is
 * loaded by a child classloader (e.g. a web app classloader).</p>
 * 
 * <p>To avoid this scenario, ensure
 * that any custom G11nLogFactory subclass is loaded by the same classloader as 
 * the base <code>G11nLogFactory</code>. Creating custom G11nLogFactory subclasses is,
 * however, rare. The standard G11nLogFactoryImpl class should be sufficient
